...
class Details extends React.Component {
    ...
    componentDidMount() {
        ...
        axios({
            method: 'GET',
            url: `http://localhost:2598/brand/${brand}`,
    ...
    handleOrder = (resId) => {
        axios({
            method: 'GET',
            url: `http://localhost:2598/menuitems/${resId}`,
            headers: { 'Content-Type': 'application/json' }
        })
            .then(response => {
    ...
    getData = (data) => {
        return fetch(`http://localhost:2598/payment`, {
            method: "POST",
            headers: {
    ...
    render() {
        const { brand, menuItems, menuItemsModalIsOpen, galleryModalIsOpen, formModalIsOpen, subTotal } = this.state;
        return (
            <div>
                <div>
                    <img src={`./${brand.image}`} alt="Sorry for the Inconvenience" width="100%" height="300px" />
                ...
                <button className="btn-order" onClick={() => this.handleOrder(brand._id)}>Place Online Order</button>
                ...
                <div className="tabs">
                    <div className="tab">
                        ...
                        <div className="content">
                            ...
                            <div className="value">{brand && brand.cuisine && brand.cuisine.map((item) => { return `${item.name}, ` })} </div>
                            <div className="head">Average Cost</div>
                            <div className="value">&#8377; {brand.min_price} for two people(approx)</div>
                        </div>
                    </div>
                    ...
                    <div className="tab">
                        ...
                        <div className="content">
                            ...
                            <div className="value">{brand.contact_number}</div>
                            <div className="head">Address</div>
                            <div className="value">{`${brand.locality}, ${brand.city}`}</div>
                        </div>
                    </div>
                </div>
                <Modal
                                    ...
                                    <div className="card" style={{ width: '43rem', margin: 'auto' }}>
                                        <div className="row" style={{ paddingLeft: '10px', paddingBottom: '10px' }}>
                                            ...
                                            <div className="col-xs-3 col-sm-3 col-md-3 col-lg-3">
                                                <img alt="" className="card-img-center title-img" src={`../${item.image}`} style={{
                                                    height: '75px',
                                                    width: '75px',
                                                    borderRadius: '20px',
                                                    marginTop: '12px',
                ...
                >
                    <div>
                        ...
                        <Carousel
                            ...
                            {brand && brand.thumb && brand.thumb.map(item => {
                                return <div>
                                    <img alt="" src={`./${item}`} style={{ width: '70%', height: '70%' }} />
                                </div>
                            })}
                        </Carousel>
                    </div>
